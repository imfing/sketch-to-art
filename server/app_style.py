...
def getPix():
    if request.method == 'POST':
        ...
        style_out = 'output/style/'+sessionId+'.jpg'
        # pix.pix_translate(input_file=file_path, output_file=pix_out)
        st.get_style_image(content=pix_out, style='styles/'+styleId+'.jpg', output_file=style_out)
        # print(pix_out)
        with open(os.path.join(os.path.dirname(__file__), style_out), 'rb') as f:
            return u"data:image/jpg;base64," + base64.b64encode(f.read()).decode('ascii')
